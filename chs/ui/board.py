...
class Board(object):
  ...
  def _generate(self, fen, board, game_over, loading=False):
    ...
    try:
      ...
      print('\n┏━━━━━━ Your Hint ━━━━━━━┓ \n   Move piece ' + starting_position + " to " + ending_position.format(
        Styles.PADDING_SMALL, Colors.PURPLE, Colors.BOLD,\
        Styles.PADDING_SMALL, Styles.PADDING_SMALL, Colors.RESET, Colors.GRAY, Colors.RESET)
      )
      #Visualize hint with piece information and position to move to
    except TypeError:
      hint_positions = None
  ...
  def reset_game(self, reset):
    if reset is Reset.RESET:
      self.generate(self.fen(), self.board, self.engine)
    return 'Board is Resetting'
